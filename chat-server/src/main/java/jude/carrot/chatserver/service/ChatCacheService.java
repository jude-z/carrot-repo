...
public class ChatCacheService {
    ...
    @Cacheable(cacheNames = "chatRoom", key = "#chatRoomId", unless = "#result.isEmpty()")
    public Optional<ChatRoom> fetchChatRoom(Long chatRoomId){
        return chatRepository.fetchChatRoom(chatRoomId);
    }

    @Cacheable(cacheNames = "chatParticipant", key = "#chatRoomId + '::' + #userId", unless = "#result.isEmpty()")
    ...
    @Cacheable(cacheNames = "chatMessage", key = "#chatMessageKey", unless = "#result.isEmpty()")
